...
public class StormcowlOnRoundStart : IAbilityTrigger
{
    ...
    public override async Task<int> TriggerAbility(Monster abilitySourceMonster, GameObject abilitySourceMonsterGameObject, MonsterAttackManager monsterAttackManager, Ability ability, bool displayLogMessage)
    {
        ...
        foreach (GameObject enemyMonster in listOfTargetMonsters.ToArray())
        {
            ...
            // if (monsterReference.monsterAffinity.element == AffinityClass.AffinityType.Electric || monsterReference.monsterSubAffinity.element == AffinityClass.AffinityType.Electric)
            // {
            //     MonsterAttack blankAttack = new MonsterAttack(ability.abilityName, EnrageAttackEffect.affinityClass, EnrageAttackEffect.effectDamageType, 0, 0, abilitySourceMonster, abilitySourceMonsterGameObject);
