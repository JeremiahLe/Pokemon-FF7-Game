...
public class AttackEffect : ScriptableObject
{
    ...
    [FormerlySerializedAs("elementClass")] [EnableIf("statToChange", StatToChange.Health)]
    public AffinityClass affinityClass;
    ...
    [FormerlySerializedAs("elementImmunity")] [EnableIf("immunityType", ImmunityType.Element)]
    public AffinityClass affinityImmunity;
    ...
    public void GrantTargetImmunity(Monster targetMonster, MonsterAttackManager monsterAttackManager, GameObject targetMonsterGameObject, string effectTriggerName)
    {
        ...
        switch (immunityType)
        {
            case ImmunityType.Element:
                monsterComponent.listOfElementImmunities.Add(affinityImmunity);
    ...
    public bool CheckPreExistingImmunity(Monster targetMonster, GameObject targetMonsterGameObject, MonsterAttackManager monsterAttackManager, ImmunityType _immunityType, AttackEffect attackEffect)
    {
        ...
        {
            case ImmunityType.Element:
                if (monsterComponent.listOfElementImmunities.Contains(attackEffect.affinityImmunity))
                {
                    monsterAttackManager.combatManagerScript.CombatLog.SendMessageToCombatLog($"{targetMonster.aiType} {targetMonster.name} is already Immune to {attackEffect.affinityImmunity.affinityType.ToString()} Element Attacks!");
                    return false;
                }
                break;
    ...
    public async Task<bool> CheckTargetIsImmune(Monster targetMonster, MonsterAttackManager monsterAttackManager, GameObject targetMonsterGameObject, AttackEffect attackEffect)
    {
        ...
        if (attackEffect.statChangeType == StatChangeType.Debuff)
        {
            ...
            if (monsterComponent.listOfElementImmunities.Contains(attackEffect.affinityClass))
            {
                monsterAttackManager.combatManagerScript.CombatLog.SendMessageToCombatLog($"{targetMonster.aiType} {targetMonster.name} is Immune to {attackEffect.affinityClass.affinityType} Element Attacks!");
                monsterComponent.CreateStatusEffectPopup("Immune!!", StatChangeType.Buff);
                return true;
            }
