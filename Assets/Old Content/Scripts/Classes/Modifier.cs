...
public class Modifier : ScriptableObject
{
        ...
        if (attackEffect.typeOfEffect == AttackEffect.TypeOfEffect.GrantImmunity)
        {
            switch (attackEffect.immunityType)
            {
                case AttackEffect.ImmunityType.Element:
                    monsterComponent.listOfElementImmunities.Remove(attackEffect.affinityImmunity);
                    monsterComponent.combatManagerScript.CombatLog.SendMessageToCombatLog($"{monsterReference.aiType} {monsterReference.name} is no longer immune to {attackEffect.affinityImmunity.affinityType} Element Attacks!", monsterReference.aiType);
                    break;
    ...
    public async Task<int> DealModifierStatusEffectDamage(Monster monsterReference, GameObject monsterReferenceGameObject)
    {
        ...
        MonsterAttack blankAttack = new MonsterAttack(modifierName, attackEffect.affinityClass, MonsterAttack.MonsterAttackDamageType.None, 1, 1, monsterReference, monsterReferenceGameObject);
